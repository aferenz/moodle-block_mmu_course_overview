 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class block_mmu_course_overview_renderer extends plugin_renderer_base {

    /**
     * Construct contents of mmu_course_overview block
     *
     * @param array $courses list of courses in sorted order
     * @param array $overviews list of course overviews
     * @return string html to be displayed in mmu_course_overview block
     */
    public function mmu_course_overview($courses, $currentyearcourses, $previousyearcourses, $nextyearcourses, $overviews) {
        $html = '';
        ...
        if (!empty($nextyearcourses)) {
        $html .= html_writer::tag('separator', get_string('separator','block_mmu_course_overview' ));
        $html .= html_writer::start_tag('a', array('href' => '#next_years_courses'));
        $html .= html_writer::tag('h3', get_string('skip_to_next_year_course','block_mmu_course_overview'));
        $html .= html_writer::end_tag('a');
        }
        ...
        if ($this->page->user_is_editing()) {
            if (ajaxenabled()) {
                ...
                $url = new moodle_url('/blocks/mmu_course_overview/move.php', array('sesskey' => sesskey()));
        ...
        $html .= html_writer::tag('h2', get_string('current_year_courses_hdr', 'block_mmu_course_overview'));
        foreach ($currentyearcourses as $key => $course) {
        ...
        // NEXT YEAR COURSES:
        if (!empty($nextyearcourses)) {
            $html .= html_writer::start_tag('div', array('id' => 'next_year'));
            $html .= html_writer::start_tag('a', array('name'=>'next_years_courses', 'id'=>'next_years_courses'));
            $html .= html_writer::end_tag('a');
            $html .= html_writer::tag('h2', get_string('next_year_courses_hdr', 'block_mmu_course_overview'));
            foreach ($nextyearcourses as $key => $course) {
                $html .= $this->output->box_start('coursebox', "course-{$course->id}");
                $html .= html_writer::start_tag('div', array('class' => 'course_title'));
                // Ajax enabled then add moveicon html
                if (!is_null($moveicon)) {
        ...
        $html .= html_writer::start_tag('a', array('name'=>'previous_years_courses', 'id'=>'previous_years_courses'));
        $html .= html_writer::end_tag('a');
    ...
    protected function activity_display($cid, $overview) {
        ...
        foreach (array_keys($overview) as $module) {
            ...
            if (get_string_manager()->string_exists("activityoverview", $module)) {
                ...
                $icontext .= get_string("activityoverview", 'block_mmu_course_overview', $modulename);
            }
    ...
    public function editing_bar_head($max = 0) {
        ...
        $options = array('0' => get_string('alwaysshowall', 'block_mmu_course_overview'));
        for ($i = 1; $i <= $max; $i++) {
            $options[$i] = $i;
        }
        $url = new moodle_url('/my/index.php');
        $select = new single_select($url, 'mynumber', $options, block_mmu_course_overview_get_max_user_courses(), array());
        $select->set_label(get_string('numtodisplay', 'block_mmu_course_overview'));
    ...
    protected function collapsible_region_start($classes, $id, $caption, $userpref = '', $default = false) {
        ...
        $this->page->requires->js_init_call('M.block_mmu_course_overview.collapsible', array($id, $userpref, get_string('clicktohideshow')));
    ...
    public function welcome_area($msgcount) {
        ...
        $output .= $this->output->heading(get_string('welcome', 'block_mmu_course_overview', $USER->firstname));

        $plural = 's';
        if ($msgcount > 0) {
            $output .= get_string('youhavemessages', 'block_mmu_course_overview', $msgcount);
        } else {
            $output .= get_string('youhavenomessages', 'block_mmu_course_overview');
            if ($msgcount == 1) {
                $plural = '';
            }
        }
        $output .= html_writer::link(new moodle_url('/message/index.php'), get_string('message'.$plural, 'block_mmu_course_overview'));
        $output .= $this->output->box_end();
